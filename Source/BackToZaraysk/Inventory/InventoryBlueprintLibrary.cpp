...
TSubclassOf<AActor> GetPickupClassForItem_Internal(const UInventoryItemData* ItemData)
{
    ...
    if (ItemData->IsA(UBackpackBaseItemData::StaticClass()))
    {
    ...
    if (ItemData->IsA(UEquipModBaseItemData::StaticClass()))
    {
        return AEquipModBase::StaticClass();
    }
    if (ItemData->IsA(UItemBaseItemData::StaticClass()))
    {
        return AItemBase::StaticClass();
    }
    if (const UEquippableItemData* Eq = Cast<UEquippableItemData>(ItemData))
    {
        // Если в DataAsset явно задан класс pickup-актора — используем его
        if (Eq->PickupActorClass)
        {
            return Eq->PickupActorClass;
        }

        // Иначе — старое поведение по типу слота
