...
public:
    ...
    // Настройки карманов: количество мини-гридов и их размер в клетках (X,Y).
    // Используются виджетом инвентаря для построения 2D-гридов карманов.
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Inventory|Pockets")
    int32 PocketMiniGridCount = 4;

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Inventory|Pockets")
    TArray<FIntPoint> PocketMiniGridSizes;
