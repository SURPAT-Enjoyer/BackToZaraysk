...
public:
    ...
    /** Обновить визуалы модов на экипированной броне (вызвать после установки/снятия мода в инвентаре). */
    UFUNCTION(BlueprintCallable, Category="Equipment")
    void RefreshEquippedArmorMods();

    /** Настроить визуал и физику выброшенного в мир предмета экипировки (броня с/без модов, жилет и т.д.). Вызывать после спавна пикапа и присвоения ItemInstance. */
    static void SetupDroppedEquipmentVisual(class AEquipmentBase* Equip, class UInventoryItemData* ItemData);
...
private:
    ...
    /** Прикрепить визуалы установленных модов к мешу брони (вызывается после экипировки брони). */
    void RefreshArmorModAttachments(USceneComponent* ArmorMeshComponent, UEquippableItemData* ArmorItemData);

    /** Прикрепить меши модов к произвольному мешу (например, на пикапе при выбросе брони). Owner — актор-владелец новых компонентов. */
    static void AttachArmorModsToMesh(AActor* Owner, USceneComponent* ArmorMeshComponent, UEquippableItemData* ArmorItemData);
