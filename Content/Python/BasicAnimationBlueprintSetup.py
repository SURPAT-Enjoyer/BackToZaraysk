...
# БАЗОВАЯ АВТОМАТИЧЕСКАЯ НАСТРОЙКА ANIMATION BLUEPRINT ДЛЯ IK
# Этот скрипт использует базовый подход без устаревшего API

def basic_animation_blueprint_setup():
    """Базовая автоматическая настройка Animation Blueprint для IK"""
    
    print("🤖 БАЗОВАЯ АВТОМАТИЧЕСКАЯ НАСТРОЙКА ANIMATION BLUEPRINT ДЛЯ IK")
    print("=" * 80)
    ...
    try:
        ...
        setup_animation_blueprint_basic()
        
        print("\n✅ БАЗОВАЯ АВТОМАТИЧЕСКАЯ НАСТРОЙКА ANIMATION BLUEPRINT ЗАВЕРШЕНА!")
...
def setup_animation_blueprint_basic():
    """Базовая настройка Animation Blueprint"""
    
    print("Базовая настройка Animation Blueprint...")
    
    # Ищем Animation Blueprint в проекте
    print("Поиск Animation Blueprint в проекте...")
    find_and_setup_animation_blueprint_basic()
    
    print("✅ Animation Blueprint настроен базово!")

def find_and_setup_animation_blueprint_basic():
    """Находит и настраивает Animation Blueprint базово"""
            ...
            for asset_data in anim_bp_assets:
                try:
                    ...
                    if anim_bp:
                        ...
                        setup_single_animation_blueprint_basic(anim_bp)
...
def setup_single_animation_blueprint_basic(anim_bp):
    """Настраивает один Animation Blueprint базово"""
    
    print("    Настройка Animation Blueprint...")
    
    # Создаем переменные IK
    print("    Создание переменных IK...")
    create_ik_variables_basic(anim_bp)
    
    # Создаем IK ноды
    print("    Создание IK нод...")
    create_ik_nodes_basic(anim_bp)
    
    print("    ✅ Animation Blueprint настроен базово!")

def create_ik_variables_basic(anim_bp):
    """Создает переменные IK в Animation Blueprint базово"""
...
def create_ik_nodes_basic(anim_bp):
    """Создает IK ноды в Animation Blueprint базово"""
...
# БАЗОВАЯ АВТОМАТИЧЕСКАЯ НАСТРОЙКА ANIMATION BLUEPRINT
basic_animation_blueprint_setup()
