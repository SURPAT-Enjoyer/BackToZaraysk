...
def setup_strafe_shift_ad():
    """Настройка системы стрейфа с комбинациями Shift + A/D (без конфликта с прыжком)"""
    print("=== НАСТРОЙКА СТРЕЙФА С КОМБИНАЦИЯМИ SHIFT + A/D ===")
    print("🎯 БЕЗ КОНФЛИКТА С ПРЫЖКОМ И ЛАЗАНИЕМ!")
    ...
    try:
        ...
        print("\n✅ НАСТРОЙКА СТРЕЙФА С КОМБИНАЦИЯМИ SHIFT + A/D ЗАВЕРШЕНА!")
...
def create_input_actions():
    ...
    actions = [
        ...
        ("IA_Shift", unreal.InputActionValueType.DIGITAL)
    ]
...
def setup_input_mapping_context():
    ...
    ia_shift = unreal.load_asset("/Game/BackToZaraysk/Core/Input/Actions/Strafe/IA_Shift.IA_Shift")
    
    if not (ia_a and ia_d and ia_shift):
        print("❌ Input Actions не найдены!")
        print("  РЕШЕНИЕ: Создайте IA_A, IA_D, IA_Shift вручную в Content Browser")
        return
    ...
    try:
        for mapping in imc_default.mappings:
            if mapping.action:
                ...
                if any("IA_A" in action_name or "IA_D" in action_name or "IA_Shift" in action_name):
    ...
    mappings_to_create = [
        ...
        (ia_shift, unreal.Key.Left_Shift, "IA_Shift")
    ]
    ...
    try:
        ...
        print("✓ IMC_Default обновлен для комбинаций Shift + A/D")
    except Exception as e:
...
def setup_blueprint_player():
    ...
    # Проверяем функции
    functions_to_check = ["Handle A Input", "Handle D Input", "Handle Shift Input"]
...
def final_verification():
    ...
    actions_to_check = ["IA_A", "IA_D", "IA_Shift"]
    ...
    print("  ✓ Space Bar остается свободным для Jump и ClimbObstacle")
    print("  ✓ Shift используется только для стрейфа")
    print("  ✓ A/D используются для движения и стрейфа (без конфликтов)")
    ...
    if input_actions_ok and imc_default and bp_player and curve:
        ...
        print("  🚫 КОНФЛИКТЫ УСТРАНЕНЫ")
        print("  🎮 УПРАВЛЕНИЕ:")
        print("    - Shift + A → Стрейф влево")
        print("    - Shift + D → Стрейф вправо")
        print("    - Space → Прыжок (без конфликтов)")
        print("    - Space → Лазание (без конфликтов)")
    else:
        print("  ⚠️ ЕСТЬ ПРОБЛЕМЫ, ТРЕБУЕТСЯ РУЧНАЯ НАСТРОЙКА")

def main():
    """Основная функция настройки"""
    print("🚀 Запуск настройки стрейфа с комбинациями Shift + A/D...")
    print("🎯 УСТРАНЕНИЕ КОНФЛИКТОВ С ПРЫЖКОМ И ЛАЗАНИЕМ!")
    
    try:
        setup_strafe_shift_ad()
        ...
        print("   - IA_Shift (Value Type: Digital bool)")
        ...
        print("   - IA_Shift Started/Completed → Handle Shift Input")
        print("4. Убедитесь, что функции Handle A/D/Shift Input вызывают Parent Function")
        print("5. Соберите проект: .\\build_ue53.bat")
        print("6. Протестируйте:")
        print("   - Shift + A → Стрейф влево")
        print("   - Shift + D → Стрейф вправо")
        print("   - Space → Прыжок (должен работать без конфликтов)")
        
        print("\n📋 ДОПОЛНИТЕЛЬНАЯ ИНФОРМАЦИЯ:")
        print("- Управление: Shift + A/D (без конфликтов)")
        print("- Shift + A → Стрейф влево")
        print("- Shift + D → Стрейф вправо")
        print("- Space остается для прыжка и лазания")
