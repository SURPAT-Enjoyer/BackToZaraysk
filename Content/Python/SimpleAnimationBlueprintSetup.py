...
# ПРОСТАЯ АВТОМАТИЧЕСКАЯ НАСТРОЙКА ANIMATION BLUEPRINT ДЛЯ IK
# Этот скрипт просто настроит Animation Blueprint для IK системы

def simple_animation_blueprint_setup():
    """Простая автоматическая настройка Animation Blueprint для IK"""
    
    print("🤖 ПРОСТАЯ АВТОМАТИЧЕСКАЯ НАСТРОЙКА ANIMATION BLUEPRINT ДЛЯ IK")
    print("=" * 70)
    ...
    try:
        ...
        setup_animation_blueprint_simple()
        
        print("\n✅ ПРОСТАЯ АВТОМАТИЧЕСКАЯ НАСТРОЙКА ANIMATION BLUEPRINT ЗАВЕРШЕНА!")
...
def setup_animation_blueprint_simple():
    """Простая настройка Animation Blueprint"""
    
    print("Простая настройка Animation Blueprint...")
    
    # Ищем Animation Blueprint в проекте
    print("Поиск Animation Blueprint в проекте...")
    find_and_setup_animation_blueprint_simple()
    
    print("✅ Animation Blueprint настроен просто!")

def find_and_setup_animation_blueprint_simple():
    """Находит и настраивает Animation Blueprint просто"""
            ...
            for asset_data in anim_bp_assets:
                try:
                    ...
                    if anim_bp:
                        ...
                        setup_single_animation_blueprint_simple(anim_bp)
...
def setup_single_animation_blueprint_simple(anim_bp):
    """Настраивает один Animation Blueprint просто"""
    
    print("    Настройка Animation Blueprint...")
    
    # Создаем переменные IK
    print("    Создание переменных IK...")
    create_ik_variables_simple(anim_bp)
    
    # Создаем IK ноды
    print("    Создание IK нод...")
    create_ik_nodes_simple(anim_bp)
    
    print("    ✅ Animation Blueprint настроен просто!")

def create_ik_variables_simple(anim_bp):
    """Создает переменные IK в Animation Blueprint просто"""
...
def create_ik_nodes_simple(anim_bp):
    """Создает IK ноды в Animation Blueprint просто"""
...
# ПРОСТАЯ АВТОМАТИЧЕСКАЯ НАСТРОЙКА ANIMATION BLUEPRINT
simple_animation_blueprint_setup()
